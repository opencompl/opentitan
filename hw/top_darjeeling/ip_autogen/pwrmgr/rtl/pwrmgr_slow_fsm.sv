...
module pwrmgr_slow_fsm import pwrmgr_pkg::*; (
  ...
  // Non-usb clocks are disabled when power is turned off.

  logic main_clk_en;
  assign main_clk_en = main_pd_ni & main_clk_en_i;
  ...
  // SEC_CM: FSM.SPARSE
  // `PRIM_FLOP_A(SlowPwrStateReset, state_d, state_q, clk_i, rst_ni)

  `PRIM_FLOP_A(state_d, state_q, SlowPwrStateReset, clk_i, rst_ni)
  ...
  `ASSERT(MainPowerOffGoesToLowPower_A,
      (!isFirstCycle
       && $past(state_q == SlowPwrStateMainPowerOff && (!main_pok_st | main_pd_ni)))
